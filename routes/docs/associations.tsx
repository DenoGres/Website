  ...
  return (
    ...
    <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5">
      ...
      <p class={description}>
        ...
        - Create ‘getter’ instance method in model A (named as “getB”)</p>
        <br/>
        <h3 class={h3}>belongsTo(A)</h3>
        <p class={description}>- Utilize (or creating if not exist) foreign key in model B’s table<br/>
          - Create ‘getter’ instance method in model B (named as “getA”)<br/><br/>
          1. For existing relationships (when relevant table constraint already exists in the database)</p><br/>
        <div class={box}>
          await Country.hasOne(Capital);<br/>
          const canada = await Country.where('name = Canada').queryInstance();<br/>
        ...
        <div class={box}>
          ...
          const ottawa = await Capital.where('name = Ottawa').queryInstance();<br/>
        ...
        <div class={box}>
          ...
          {'}'}<br/>
          <br/>
          interface Profile {'{'}<br/>
          &nbsp;&nbsp;id:number;<br/>
          &nbsp;&nbsp;email:string;<br/>
          &nbsp;&nbsp;address?:string;<br/>
          {'}'}<br/>
          class Profile extends Model {'{'}<br/>
          &nbsp;&nbsp;static table = 'profiles';<br/>
          &nbsp;&nbsp;static columns = {'{'}<br/>
          &nbsp;&nbsp;&nbsp;&nbsp;id: {'{'} type:'number', primaryKey: true, autoIncrement:true {'}'},<br/>
          &nbsp;&nbsp;&nbsp;&nbsp;email: {'{'} type:'string', notNull: true {'}'},<br/>
          &nbsp;&nbsp;&nbsp;&nbsp;address: {'{'} type:'string', notNull: false {'}'},<br/>
          &nbsp;&nbsp;{'}'}<br/>
          {'}'}<br/>
        </div>
        <br/>
        ...
        <p class={description}>This will execute following command to your database:</p>
          ...
          ON DELETE SET NULL ON UPDATE CASCADE;</div>
        <br/>
        <p class={description}>Creating new association record (beta)</p>
        <div class={box}>
          const foo = await User.where('firstname = foo').queryInstance();<br/>
          <br/>
...
For existing association in the database (e.g. existing foreign key constraints), invoking these method will create ‘getter’ method to the model.</p>
        ...
        <div class={box}>
          ...
          <p class={comment}>// getter method name will be by default : get + target model's name</p>
          ...
          &nbsp;&nbsp;&nbsp;&nbsp;hair_colors: "blonde, brown, black, red",<br/>
          &nbsp;&nbsp;&nbsp;&nbsp;skin_colors: "caucasian, black, asian, hispanic",<br/>
          &nbsp;&nbsp;&nbsp;&nbsp;eye_colors: "brown, blue, green, hazel, grey, amber",<br/>
        ...
        <div class={box}>
          ...
          const droid = await Species.where('name = Droid').queryInstance();<br/>
          const droidCharacters = await droid.getPersons();<br/>
          <p class={comment}>// getter method name will be by default : get + target model's name  + 's' (for plural)</p>
        ...
        <div class={box}>
          ...
          {'}'}<br/>
          interface Team {'{ ... }'}<br/>
          class Team extends Model {'{'}<br/>
          &nbsp;&nbsp;static table = 'teams';<br/>
          &nbsp;&nbsp;static columns = {'{'}<br/>
          &nbsp;&nbsp;&nbsp;&nbsp;id: {'{'} type:'number', primaryKey: true, autoIncrement:true {'}'},<br/>
          &nbsp;&nbsp;&nbsp;&nbsp;teamName: {'{'} type:'string', notNull: true {'}'},<br/>
          &nbsp;&nbsp;&nbsp;&nbsp;teamRole: {'{'} type:'string', notNull: false {'}'},<br/>
          &nbsp;&nbsp;{'}'};<br/>
          {'}'}
        </div>
        <br/>
...
For existing Many-To-Many association in the database, you need to specify the model representing the cross-table.</p>
        <div class={box}>
	  import {'{'} manyToMany {'}'} from 'https://deno.land/x/denogres/mod.ts'<br /><br />
        ...
        &nbsp;&nbsp;&nbsp;&nbsp;release_date: 1977-05-25T07:00:00.000Z<br/>
        &nbsp;&nbsp;{'}'},<br/>
        &nbsp;&nbsp;{'{'}<br/>
        &nbsp;&nbsp;&nbsp;&nbsp;_id: 2,<br/>
        ...
        &nbsp;&nbsp;&nbsp;&nbsp;producer: "Gary Kurtz, Rick McCallum",<br/>
        &nbsp;&nbsp;&nbsp;&nbsp;release_date: 1980-05-17T07:00:00.000Z<br/>
        &nbsp;&nbsp;{'}'},<br/>
        &nbsp;&nbsp;{'{ .... (more records...) ... }'}<br/>
        ]
        </div>
